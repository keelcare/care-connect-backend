...
if echo "$REFRESH_REQ" | grep -q "HTTP/1.1 200"; then
  ...
  if echo "$REFRESH_REQ" | grep -qi "set-cookie: access_token="; then
    echo "SUCCESS: New access token was issued via Set-Cookie."
  else
    echo "FAILED: New access token NOT found in response headers."
    echo "$REFRESH_REQ" | grep -i "set-cookie"
  fi
else
  echo "FAILED: Transparent refresh failed. Endpoint returned non-200."
  echo "$REFRESH_REQ" | head -n 15
  echo -e "\nError Body:"
