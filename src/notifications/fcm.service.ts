...
export class FcmService implements OnModuleInit {
  ...
  private initializeFirebase() {
    try {
      ...
      if (!projectId || !clientEmail || !privateKey) {
        this.logger.warn(
          "Firebase Admin credentials missing. Push notifications will be disabled.",
        );
        return;
      }
  ...
  async sendPushNotification(
    ...
    if (!this.initialized) {
      this.logger.debug(
        "Skipping push notification: Firebase Admin is not initialized.",
      );
      return false;
    }
    ...
    try {
      ...
      this.logger.log(
        `Successfully sent push notification. Message ID: ${messageId}`,
      );
      return true;
    } catch (error) {
      this.logger.error(
