...
export class AdminWhatsAppController {
    ...
    @ApiQuery({ name: 'search', required: false, type: String, description: 'Name, phone or email' })
    ...
    async getEnquiry(@Param('id', ParseUUIDPipe) id: string) {
        ...
        const messages = await this.prisma.whatsapp_messages.findMany({
            ...
            select: { id: true, direction: true, message_body: true, created_at: true },
        });
    ...
    async replyToEnquiry(
        ...
        const enquiry = await this.prisma.whatsapp_enquiries.findUnique({ where: { id } });
        if (!enquiry) throw new NotFoundException('Enquiry not found');
