...
export class WhatsAppBotService {
    ...
    async handleIncomingMessage(
        ...
        if (exists) {
            this.logger.warn(`Duplicate webhook event for message_id ${messageId}, skipping.`);
            return;
        }
    ...
    private async processStep(
        ...
        switch (step) {
            case WhatsAppConversationStep.WELCOME:
                await this.advance(phoneNumber, WhatsAppConversationStep.COLLECT_NAME, {}, {
                    name: null,
                });
            ...
            case WhatsAppConversationStep.COLLECT_NAME:
                ...
                await this.advance(phoneNumber, WhatsAppConversationStep.COLLECT_PHONE, { name: text });
                await this.sendAndLog(
                    phoneNumber,
            ...
            case WhatsAppConversationStep.COLLECT_PHONE:
                ...
                if (!phoneRegex.test(text)) {
                    await this.sendAndLog(phoneNumber, `That doesn't look like a valid phone number. Please try again, e.g. +91 9876543210`);
                    return;
                }
                await this.advance(phoneNumber, WhatsAppConversationStep.COLLECT_EMAIL, {
            ...
            case WhatsAppConversationStep.COLLECT_CATEGORY:
                ...
                if (isNaN(categoryIndex) || categoryIndex < 0 || categoryIndex >= ENQUIRY_CATEGORIES.length) {
                    await this.sendAndLog(
                        phoneNumber,
            ...
            case WhatsAppConversationStep.COLLECT_ENQUIRY:
                if (text.length < 3) {
                    await this.sendAndLog(phoneNumber, `Please provide a brief description of your query.`);
                    return;
                }
    ...
    private async finalizeEnquiry(
        ...
        await this.prisma.whatsapp_conversations.update({
            where: { phone_number: phoneNumber },
            data: { current_step: WhatsAppConversationStep.COMPLETED, status: 'COMPLETED' },
        });
        ...
        this.logger.log(`Enquiry created for phone ${phoneNumber} (category: ${category})`);
    }
